...
void print_tensor(memory t, std::string name) {
  ...
  auto ndims = (size_t)memory_pd.desc().data.ndims;
...
void run(int seq_length_max , int batch, int input_size, int feature_size){
	...
	memory::dims common_weights_iter_dims = {layers, direc, gates, feature_size, feature_size};
	memory::dims common_weights_layer_dims = {layers, direc, gates, feature_size, input_size};
    ...
    auto user_common_weights_layer_memory
        = mkldnn::memory({ formatted_md(common_weights_layer_dims,
                           memory::format::ldgoi), cpu_engine },
  ...
  print_tensor(user_common_weights_iter_memory, "user_common_weights_iter_memory");
  print_tensor(common_weights_iter_memory, "common_weights_iter_memory");
...
int main(int argc, char **argv) {
  if (argc != args_count) {
    ...
    oss << "Usage: " << argv[0] << " [N] [T] [I] [H]"<<std::endl;
